      ...
      <div className="px">
        ...
        <div style={{ marginBottom: 20 }}>
          <div className="between" style={{ marginBottom: 12 }}>
            <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--ink)' }}>Agenda de hoy</div>
            <a href="/panel/agenda" style={{ fontSize: 13, color: 'var(--emerald)', fontWeight: 600, textDecoration: 'none' }}>Ver todo</a>
          </div>
          {!loading && appointments.length === 0 ? (
            ...
            <div className="card" style={{ padding: '2px 14px' }}>
              {(loading ? Array(3).fill(null) : appointments).map((appt, i) => (
                <div key={appt?.id ?? i} style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '13px 0', borderBottom: i < (loading ? 2 : appointments.length - 1) ? '1px solid var(--line)' : 'none' }}>
                  {loading ? (
                    <>
                    ...
                    <>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--ink)' }}>{appt.patient?.name ?? 'Paciente'}</div>
                        <div style={{ fontSize: 12.5, color: 'var(--muted)', marginTop: 2 }}>
                          {appt.time} · {appt.duration_min} min
                        </div>
                      </div>
                      <span className={`chip chip--dot ${(STATUS_STYLE[appt.status] ?? STATUS_STYLE.pendiente).cls}`} style={{ padding: '4px 10px', fontSize: 11 }}>
